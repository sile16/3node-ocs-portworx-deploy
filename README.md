...
| 2026-04-16 | `1b9b179` | 4.21.9 | 3.6.0    | libvirt + SB + **RUNBOOK-3 validated** | 986 GiB (2×493) | Full RUNBOOK-3 validation from `px-installed` snapshot using exact documented commands. Fixed `oc wait` pattern: poll-until-exists (`until oc get deploy ... ; do sleep 5; done`) before `oc wait --for=condition=Available` — `oc wait` exits NotFound immediately if the resource doesn't exist yet. HCO apply succeeded on **first try** (exit code 0, zero webhook errors). Total RUNBOOK-3 time: ~9 min (SCs → CNV operator → HCO → test VM → live migration master-1 → master-2). All auto-resolved: SC=`px-rwx-block-kubevirt`, RWX, Block. Zero boot images. |
