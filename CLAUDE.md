...
- PX `systemMetadataDevice` / `kvdbDevice` values must be **raw device paths** (`/dev/vda5`, `/dev/sda5`, …) — PX 3.6.0 has a symlink-resolution bug that breaks partlabel/by-id symlinks. `deploy/templates/04-prepare-for-portworx.sh` resolves those per node via `oc debug` and patches `06-` in place. Partlabel is still correct for the MC partition definitions and for any non-PX consumer.
